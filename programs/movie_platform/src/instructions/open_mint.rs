...
pub struct OpenMint<'info> {
    ...
    #[account(
        ...
        constraint = platform_config.authority == authority.key() @ PlatformError::Unauthorized
    )]
...
pub fn handler(ctx: Context<OpenMint>, mint_open_timestamp: i64) -> Result<()> {
    ...
    // Security: Prevent setting negative timestamps
    require!(
        mint_open_timestamp >= 0,
        PlatformError::InvalidTimestamp
    );
